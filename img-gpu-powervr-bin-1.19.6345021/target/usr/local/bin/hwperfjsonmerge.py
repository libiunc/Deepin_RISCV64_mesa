...
class Merger(object):
    ...
    def _merge(self, devid):
        ...
        if devid in self._to_process:
            host_process = self._to_process[devid][0]
            if host_process.is_raw():
                assert "hwperf_packets" in host_process.get_json(), \
                    "hwperf_packets key not found within json"

                root_key = "hwperf_packets"
                ts_key = "rgxtimer"
            else:
                assert "timeperiods" in host_process.get_json(), \
                    "timeperiods key not found within json"

                root_key = "timeperiods"
                ts_key = "start"
